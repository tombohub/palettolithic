...
import { frameworkModule } from "../frameworks";
 ...
 */
function createPalette(input: CreatePalleteInputDto): CreatePaletteOutputDto {
  // collect framework setup data
  const originalPalette = frameworkModule.getOriginalPalette(input.framework);
  ...
  const code = frameworkModule.generateConfigurationCode(
    input.framework,
    newPalette
  );
 ...
 */
function initializeState(): InitialStateDto {
...
export const appService = { initializeState, createPalette };
